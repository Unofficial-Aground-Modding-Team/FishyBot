...
from database.sqlite_model import Game
...
from blueprints.modio.operations import (
    get_game,
    get_mod,
    get_mods,
    get_comment,
...
def handle_error(listener: ListenerRecord, reason: str, error):
    ...
    errors_db.put(str(uuid.uuid4()), {"reason": reason, "error": error, "listener": listener.key})
    try:
...
# @comment_commands.command("run", "Temporary command for testing")
@bp.task()
def get_new_comments(*_):
    NEW_LAST_UPDATED = math.floor(time.time())
    listeners = [listener for listener in listeners_db.fetch() if not listener.disabled]
    if not listeners:
    ...
    for listener in listeners:
        try:
            ...
            # breakpoint()
            listener.disabled = True
            errors_db.put(str(uuid.uuid4()), {"reason": "webhook failed to send", "listener": listener.key, "error": str(err)})
