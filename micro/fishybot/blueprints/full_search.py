...
from fishybot.common.exceptions import GameNotFoundError, RecordNotFoundError
from fishybot.database.sqlite_model import GameRecord
from fishybot.database.operations import get_possible_games, get_possible_tags, get_exact_record, get_matching_records
...
def dropdown_callback(ctx: Context):
    ...
    record = get_exact_record(game_, mod_, tag_, id_, include_xml=True, include_hidden=True)
    message = "```xml\n" + record.xml.strip() + "\n"
    if len(message) >= 1990:
        NL = "\n"
...
def search_ih(
    ...
    if isinstance(id, str) and "@mod@" in id:
        ...
        try:
            records = [get_exact_record(game, mod, tag, id, include_xml=True, include_hidden=True)]
