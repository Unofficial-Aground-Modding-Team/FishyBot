...
def get_game(game_id: str, force_update: bool = False) -> GameRecord:
    if not force_update:
        record = modio_games_db.get(f'game_{game_id}')
    if force_update or record is None:
...
def get_mods(game_id: int, filters: dict[str, str]) -> Modio_Mods:
    ...
    records = [
        ModRecord(modio_mod=mod, last_updated=last_updated, name=mod.name.casefold()) 
